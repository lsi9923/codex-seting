...
경로는 모두 **현재 스킬 폴더 기준 상대 경로**다:

- `references/template.md` — design.md 16섹션 템플릿 (Step 5에서 Read)
- `references/report-prompt.md` — HTML 리포트 생성 규칙 (Step 6에서 Read)
- `references/pitfalls.md` — 14가지 함정 (Step 4에서 Read)
- `examples/stripe/design.md` — 골드 스탠다드 예시 (Step 5에서 참조)
- `examples/stripe/report.ko.html` — 리포트 골드 스탠다드 (Step 6에서 참조)
...
# 브랜드 색상 후보
python "scripts/brand_candidates.py" {slug}

# CSS var() 체인 해결
python "scripts/var_resolver.py" {slug}

# 타이포그래피 스케일
python "scripts/typo_extractor.py" {slug}

# 시멘틱 alias 계층 분류
python "scripts/alias_layer.py" {slug}
```
...
5. Read: `references/pitfalls.md` — 14가지 함정
...
1. Read: `references/template.md` — 16섹션 v2.0
2. Read: `examples/stripe/design.md` — 골드 스탠다드 (구조 참조)
...
2. Read: `examples/stripe/report.ko.html` — 골드 스탠다드 (CSS/JS/구조 복사)
