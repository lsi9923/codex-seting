...
# 또는 현재 스킬 폴더 기준으로 상대 경로를 계산해 사용

# Bash 예시용 가상환경 활성화 경로
VENV="<프로젝트>/.venv/bin/activate"

# Python 실행 파일 (프로젝트에 맞게 설정)
# Windows PowerShell 예: .\.venv\Scripts\python.exe
# macOS/Linux 예: ./.venv/bin/python
PYTHON="<프로젝트 가상환경의 python 경로>"
```

예시 명령은 bash 기준이다. PowerShell에서는 동일한 스크립트를 프로젝트 가상환경의 Python(`$PYTHON`)으로 실행한다. `lxml` 패키지가 필요하다.
...
7. **venv 사용**: 프로젝트 가상환경의 Python 실행 파일 사용 (`.venv\Scripts\python.exe` 또는 `.venv/bin/python`, `lxml` 필요)
...
13. **examples 폴더 미사용**: 존재하지 않는 예제 경로를 가정하지 말고 실제 포함된 `scripts/`, `references/`, `templates/`만 사용
